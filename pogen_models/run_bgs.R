...
recrates = subset(recrates, recrates$species == spec, select=c("chr", "window.id", "rho", "pi", "fd", "use", "filt", "ratemtd"));
...
setkeyv(bgs.use, c("sh", "P", "filt", "ratemtd"))
bgs.use=bgs.use[complete.cases(bgs.use),]

sel.full.fd<-bgs.use[,get_full_fd(pi,G,rho,fd),by="sh,P,filt,ratemtd"]
sel.full.fd$model="full(hh_var)"
sel.full.nofd<-bgs.use[,get_full_nofd(pi,G,rho),by="sh,P,filt,ratemtd"]
sel.full.nofd$model="full(hh_const)"
sel.hh.fd<-bgs.use[,get_hh_fd(pi,rho,fd),by="sh,P,filt,ratemtd"]
sel.hh.fd$model="hh_only(var)"
sel.hh.nofd<-bgs.use[,get_hh_nofd(pi,rho),by="sh,P,filt,ratemtd"]
sel.hh.nofd$model="hh_only(const)"
sel.bgs.fd<-bgs.use[,get_bgs_fd(pi,G),by="sh,P,filt,ratemtd"]
sel.bgs.fd$model="bgs_only(var)"
sel.bgs.nofd<-bgs.use[,get_bgs_nofd(pi,rho),by="sh,P,filt,ratemtd"]
sel.bgs.nofd$model="bgs_only(const)"
sel.neut<-bgs.use[,get_neut(pi),by="sh,P,filt,ratemtd"]
